...
class CatPromocionalControllerCatCcotizacions extends JControllerForm
{
    ...
    public function generateOrdersFile() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('L'.$ind, 'profesional_usuario')
            ->setCellValue('M'.$ind, 'fechaproducto_usuario')
			
			->setCellValue('N'.$ind, 'nombre_cliente')
            ->setCellValue('O'.$ind, 'email_cliente')
            ->setCellValue('P'.$ind, 'empresa_cliente')
            ->setCellValue('Q'.$ind, 'ciudad_cliente')
            ->setCellValue('R'.$ind, 'telefono_cliente')
			->setCellValue('S'.$ind, 'canal_cliente')
			->setCellValue('T'.$ind, 'comentario_cliente')
            ->setCellValue('U'.$ind, 'estado_cotizacion');        
        // Loop ProductsList
        foreach($orders as $order) {
            ...
            ->setCellValue('L'.$ind, ($order->profesional_usuario==1)?'SI':'NO')
            ->setCellValue('M'.$ind, $order->fechaproducto_usuario)

            ->setCellValue('N'.$ind, $order->nombre_cliente)
            ->setCellValue('O'.$ind, $order->email_cliente)
            ->setCellValue('P'.$ind, $order->empresa)
            ->setCellValue('Q'.$ind, $order->nombre_ciudad)
            ->setCellValue('R'.$ind, $order->telefono)
			->setCellValue('S'.$ind, $order->nombre_canal)
			->setCellValue('T'.$ind, $order->comentario)
            ->setCellValue('U'.$ind, $order->nombre_estado_cotizacion);
        }
        // Renombrar Hoja
