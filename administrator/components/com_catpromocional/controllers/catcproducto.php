...
class CatPromocionalControllerCatCproducto extends JControllerForm
{
    ...
    public function generateProductsFile() {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C'.$ind, 'product_s_desc')
            ->setCellValue('D'.$ind, 'product_desc')
            ->setCellValue('E'.$ind, 'product_publish')

            ->setCellValue('F'.$ind, 'product_special')
            ->setCellValue('G'.$ind, 'product_name')
            ->setCellValue('H'.$ind, 'proveedor')
            ->setCellValue('I'.$ind, 'cat.Principal')
            ->setCellValue('J'.$ind, 'cat.Secundaria')
            ->setCellValue('K'.$ind, 'TSL')
            ->setCellValue('L'.$ind, 'Restricción')
            ->setCellValue('M'.$ind, 'id_escala')
        ...
        foreach($products as $product) {
            ...
            ->setCellValue('C'.$ind, $product->product_s_desc)
            ->setCellValue('D'.$ind, $product->product_desc)
            ->setCellValue('E'.$ind, ($product->product_publish == 1)?'Y':'N')

            ->setCellValue('F'.$ind, $product->destacado)
            ->setCellValue('G'.$ind, $product->nombre)
            ->setCellValue('H'.$ind, $product->proveedor)
            ->setCellValue('I'.$ind, $product->cat_principal)
            ->setCellValue('J'.$ind, $product->cat_secundaria)
            ->setCellValue('K'.$ind, $product->modelo)
            ->setCellValue('L'.$ind, $product->restriccion)
            ->setCellValue('M'.$ind, $product->id_escala)
            ->setCellValue('N'.$ind, $product->estado);
        }
        // Renombrar Hoja
