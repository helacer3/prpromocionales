 ...
 */
class CatPromocionalModelCatCcostecnica extends JModelAdmin
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_catpromocional.Catccostecnica', 'catccostecnica',
    ...
    /**
     * Method to insert Product Technique Cost Tsl.
     *
     * @return  
     * @since   2.5
     */
    public function insertProductTechniqueCostTsl($db,$arrData) {
       // Create a new query object.
        $query = $db->getQuery(true);
        // Prepare the insert query.
        $query
            ->insert($db->quoteName('#__producto_tecnica_costo_tsl'))
     ...
     * @since   2.5
     */
    public function insertProductTechniqueCost($db) {
       // Create a new query.
        $query = "INSERT INTO prp_producto_tecnica_costo
            ...
            FROM prp_producto_tecnica_costo_tsl ptpt
    ...
    /**
     * Method to get Costs Techniques List.
     *
     * @return  
     * @since   2.5
     */
    public function getCostsTechniquesList() {
        // Create a new query object.       
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        // query
        $query
            ...
            ->join('LEFT', $db->quoteName('#__producto_tecnica_costo', 'ptc') 
                . ' ON (' . $db->quoteName('pt.id') . ' = ' . $db->quoteName('ptc.id_producto_tecnica') 
                . ' AND ' . $db->quoteName('er.id') . ' = ' . $db->quoteName('ptc.id_escala_rangos')
                . ' AND ' . $db->quoteName('u.id') . ' = ' . $db->quoteName('ptc.id_unidad') . ')'
            )
            //->where('ptp.valor > 0 ')  
