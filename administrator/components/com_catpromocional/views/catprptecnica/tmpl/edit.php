...
$scriptJs = "
    jQuery(document).ready(function(){
        // Products
        jQuery('#jform_id_producto').on('change', function() {
            var id = jQuery(this).val();
            jQuery.ajax({
                url : 'index.php?format=raw&option=com_catpromocional&task=getTechniquesByProduct',
                type: 'POST',
                data: {id: id},
                success: function(response) {
                    //alert(response);
                    jQuery('#jform_id_producto_tecnica').empty().append(response);
                }
            });
        });
        // Techniques
        jQuery('#jform_id_producto_tecnica').on('change', function() {
            var id = jQuery(this).children(':selected').attr('id');
            jQuery.ajax({
                url : 'index.php?format=raw&option=com_catpromocional&task=getUnitsByTechnique',
                type: 'POST',
                data: {id: id},
                success: function(response) {
                    //alert(response);
                    jQuery('#jform_id_unidad').empty().append(response);
                }
            });
        });
        // Scales
        ...
        if(parseInt(jQuery('#jform_id_producto').val()) > 0) {
            jQuery('#jform_id_producto').trigger('change');
        }        
...
<form action="<?php echo JRoute::_('index.php?option=com_catpromocional&layout=edit&id=' . (int) $this->item->id); ?>"
    ...
    <div class="form-horizontal">
        <fieldset class="adminform">
            <legend class="PRtitle"><?php echo JText::_('COM_CATPROMOCIONAL_CATPROMOCIONAL_PRPTECNICADETAILS'); ?></legend>
            <div class="row-fluid">
                <div class="span6">
