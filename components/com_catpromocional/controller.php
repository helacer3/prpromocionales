...
require_once JPATH_COMPONENT_ADMINISTRATOR.'/class/restclient.php';
/**
...
class CatPromocionalController extends JControllerLegacy
{
    ...
    public function getqconfirm() {
        ...
        if ($user->guest) {            
            ...
            // Get Countries.
            $countries  = $modelg->getCountries();
            // Get User Require Data Speed.
            $userSpeed  = $this->validateUserSpeed($user);
            // Get View.
            ...
            $view->assignRef('userSpeed', $userSpeed);
            $view->display();
        }
    }
    ////////////////////////////////////////////////////////////////////////////
    ...
    // send Sms Message
    ////////////////////////////////////////////////////////////////////////////
    private function sendSmsMessage($user) {
        // get Params Sms
        $smsParams = $this->getParamsSms();
        // Validate Module Sms Active
        if ($smsParams->sms_active == 1) {
            // validate User Is Loggued
            if ($user && !$user->guest) {
                try {                    
                    // Get Models Quotation.
                    $modelq      = $this->getModel('quotation');
                    ...
                    // get user Phone
                    $userPhone   = (array_key_exists('phone2',$userProfile->profile))?$userProfile->profile['phone2']:'';
                    // validate User Phone
                    if ($userPhone != "" && strlen($userPhone) == 10) {
                        $api     = new RestClient([
                            'base_url' => $smsParams->sms_url
                        ]);
                        $result  = $api->get("",
                            array(
                                'action'      => 'sendmessage',
                                'username'    => $smsParams->sms_username,
                                'password'    => $smsParams->sms_password,
                                'recipient'   => '57'.$userPhone,
                                'messagedata' => $this->unicode2html($smsParams->sms_message),
                                'longMessage' => "false"
                            )
                        );
                        // echo "<pre>"; print_r($userProfile); echo "</pre>"; die;
                        // register Log Send Sms
                        $modelq->registerLogSendSms($user,$userPhone,$result->response);
                        //var_dump($result->response);die;
                        // validate Status
                        if ($result->info->http_code == 200) {}
                    }
                } catch(\Exception $ex) {
                    echo "Ocurrió el siguiente error: ".$ex->getMessage();die;
                }
            }
        }
        // default Return
    ...
    // unicode2html
    ////////////////////////////////////////////////////////////////////////////    
    private function unicode2html($originalString){
        return iconv(mb_detect_encoding($originalString, mb_detect_order(), true), "UTF-8", $originalString);
    }
    ////////////////////////////////////////////////////////////////////////////
    // get Params Sms
    ////////////////////////////////////////////////////////////////////////////
    private function getParamsSms() {
        jimport( 'joomla.application.module.helper' );
        $module = &JModuleHelper::getModule( 'mod_smscotizacion' );
    ...
    public function sendMailQuotation() {
        ...
        // $arrCC = explode(";",$quotext->correo_destino1);
        // clean arrCC
    ...
    public function updateInfoUser() {
        ...
        $user = JFactory::getUser();
        // delete User Profile
		if ($this->validateUserSpeed($user)) {
			if($this->deleteUserProfile($user)) {
				$this->InsertOtherInfoUser($user);
			}
		}
    }
    ////////////////////////////////////////////////////////////////////////////
    // Delete User Profile
    ////////////////////////////////////////////////////////////////////////////
    private function deleteUserProfile($user) {
        $db = JFactory::getDbo();         
        $query = $db->getQuery(true);         
        $query->delete($db->quoteName('#__user_profiles'));
        $query->where($db->quoteName('user_id')     . '  = '.(int)$user->id);
        $query->where($db->quoteName('profile_key') . ' != '.'\'profile.phone\'');
        $db->setQuery($query);         
        $db->execute();
        return true;
    }
    ////////////////////////////////////////////////////////////////////////////
    // Insert other info user.
    ////////////////////////////////////////////////////////////////////////////
    private function InsertOtherInfoUser($user,$formData) {
        // Get Input
        $input = JFactory::getApplication()->input;
        $this->insertUserProfile($user, 'profile.company',       $input->getString('jfo_company',''),1);
        $this->insertUserProfile($user, 'profile.country',       $input->getInt('jfo_country',0),2);
        $this->insertUserProfile($user, 'profile.city',          $input->getString('jfo_city',''),3);
        $this->insertUserProfile($user, 'profile.phone2',        $input->getString('jfo_phone2',''),4);
        $this->insertUserProfile($user, 'profile.fax',           $input->getString('jfo_fax',''),6); 
        $this->insertUserProfile($user, 'profile.sector',        $input->getInt('jfo_sector',0),7);
        $this->insertUserProfile($user, 'profile.profesional',   $input->getInt('jfo_profesional',0),8);
        $this->insertUserProfile($user, 'profile.fechaProducto', $input->getString('jfo_fecha',''),9);
        $this->insertUserProfile($user, 'profile.userexpress', 1,99);
        return true;
    }
    ////////////////////////////////////////////////////////////////////////////
    // Insert user profile.
    ////////////////////////////////////////////////////////////////////////////
    private function insertUserProfile($user,$field,$value,$order) {
        // Get a db connection.
        $db = JFactory::getDbo();
        // Get user
        //$user = JFactory::getUser();
        ...
        // Insert values.
        $values = array($user->id,$db->quote($field),$db->quote($value),$order);
    ...
    ////////////////////////////////////////////////////////////////////////////
    // validate User Created As Speed
    ////////////////////////////////////////////////////////////////////////////
    private function validateUserSpeed($user) {
		$db    = JFactory::getDbo();
		$query = $db->getQuery(true); 
		$query->select('profile_value')
		   ->from($db->quoteName('#__user_profiles')) 
		   ->where($db->quoteName('user_id') . ' = '. (int)$user->id)
		   ->where($db->quoteName('profile_key') . ' = '.'\'profile.userexpress\''); 
		$db->setQuery($query); 
		$valor = $db->loadResult();
		//echo "<br />El valor es: ".$valor;
		if ($valor == 2) return true; return false;
	}
}