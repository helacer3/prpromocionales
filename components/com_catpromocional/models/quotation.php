...
class CatpromocionalModelQuotation extends JModelLegacy
{
    ...
    public function createQuotation($sesQuotation,$arrform,$modelqp,$modeldb)
    {
        ...
        $columns = array('id_usuario','nombre_cliente','email_cliente','comentario',
            ...
            'empresa','id_ciudad','telefono');
        // Insert values.
        $values = array($user->id, $db->quote($arrform['name']), 
            $db->quote($arrform['email']), $db->quote($arrform['nota']), 
            (int)$arrform['tled'], $db->quote($arrform['rlead']), 
            (int)$arrform['pfuente'], (int)$arrform['pcanal'], $db->quote(date("Y-m-d",
            strtotime($arrform['fpfuente']))),'NOW()',0,1,
            $db->quote($arrform['empresa']),(int)$arrform['ciudad'],
            $db->quote($arrform['telefono']));
        // generate Register.
    ...
    ////////////////////////////////////////////////////////////////////////////
    // register Log Sens Sms
    ////////////////////////////////////////////////////////////////////////////
    public function registerLogSendSms($user,$userPhone,$response) {
        try {
            // Get a db connection.
            $db = JFactory::getDbo();       
            ...
            // Insert columns.
            $columns = array('user_id', 'user_phone','response', 'fecha');
            // Insert values.
            $values = array((int)$user->id, $userPhone, "\"".$response."\"", 'NOW()');
            // Prepare the insert query.
            $query
                ->insert($db->quoteName('#__log_sendsms'))
